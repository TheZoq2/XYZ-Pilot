...
z cos(a) cos(b)+y sin(a) cos(b)-x sin(b)
	...
	\textbf{STOREOBJ} - Lagring av data i objektminnet, omedelbar adressering\\
		\quad Operation: OBJMEM[DATA]=REG1\\
		\quad Syntax: STOREOBJ REG1 DATA \\
        \textbf{STOREOBJ.R} - Lagring av data i objektminnet, indexerad adressering\\
                \quad Operation: OBJMEM[DATA+REG2]=REG1\\
                \quad Syntax: STOREOBJ.R REG1 REG2 DATA\\
	...
	\textbf{WAITFRAME} - Vänta på att VGA-motorn renderat en bild\\
        ...
        \textbf{DBG} - Ändrar det nuvarande värdet på segment-displayen\\
                \quad Operation: Visar de 16 minst signifikanta bitarna i REG1 på
                      segment-displayen\\
                \quad Syntax: DBG REG1\\
    ...
    \textbf{LEN} - Beräknar längen av en vektor \\
        \quad Operation: REG1=len(REG2)\\
		\quad Syntax: LEN REG1 REG2\\
    \textbf{DOT} - Beräknar skalärprodukten av en vektor \\
        \quad Operation: REG1=dot(REG2)\\
		\quad Syntax: DOT REG1 REG2\\
    \textbf{MULCOS} - Beräknar cosinusvärdet av en vinkel och multiplicerar det med ett heltal \\
        \quad Operation: REG1=cos(REG2) * REG3\\
		\quad Syntax: DOT REG1 REG2 REG3\\
